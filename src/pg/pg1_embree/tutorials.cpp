...
{
	std::cout << "[TUTORIAL] Starting Ray Tracer with VDB support..." << std::endl;

	try {
		RayTracer rayTracer(640, 480, deg2rad(45.0f),
			Vector3(15.f, 0.f, 5.f), Vector3(0.f, 0.f, 0.f), Vector3(0.f, 200.f, 1000.f), config);

		std::cout << "[TUTORIAL] Ray Tracer initialized successfully" << std::endl;
		std::cout << "[TUTORIAL] Use UI to load VDB volumes or OBJ models" << std::endl;

		// Start main rendering loop
		rayTracer.MainLoop();

		std::cout << "[TUTORIAL] Ray Tracer finished" << std::endl;
	}
	catch (const std::exception& e) {
		std::cerr << "[TUTORIAL ERROR] Exception caught: " << e.what() << std::endl;
		return EXIT_FAILURE;
	}
	catch (...) {
		std::cerr << "[TUTORIAL ERROR] Unknown exception caught" << std::endl;
		return EXIT_FAILURE;
	}
